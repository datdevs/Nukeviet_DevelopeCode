...
$structure_upload = isset($array_config['structure_upload']) ? $array_config['structure_upload'] : 'Y';
...
if ($nv_Request->isset_request('add', 'get') or $nv_Request->isset_request('edit', 'get')) {
    ...
    if ($nv_Request->isset_request('submit', 'post')) {
        ...
        if (empty($data['alias'])) $data['alias'] = change_alias($data['title']);
        ...
        if (empty($error)) {
            ...
            if ($id > 0) {
                ...
                homeimgalt, front, road, structure, type, provinceid, districtid, wardid, address, maps, inhome, allowed_comm, hitstotal, showprice, contact_fullname, contact_email, contact_phone, contact_address, ordertime)
				VALUES (:title, :alias, :catid, :hometext, :bodytext, :admin_id, ' . NV_CURRENTTIME . ', ' . NV_CURRENTTIME . ', :exptime,
				:code, :area, :size_v, :size_h, :price, :price_time, :money_unit, :typeid, :projectid, :way_id, :legal_id, :homeimgfile, ' . $data['homeimgthumb'] . ', :homeimgalt, :front, :road, :structure, :type, :provinceid, :districtid, :wardid, :address, :maps, :inhome,
