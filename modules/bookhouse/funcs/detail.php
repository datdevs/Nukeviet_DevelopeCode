...
if ($array_config['othertype'] == 0) {
    $where = 'id != ' . $id . ' AND catid = ' . $data_content['catid'] . ' AND status=1 AND status_admin=1 AND is_queue=0';
...
$db->sqlreset()
    ->select('id, catid, title, alias, code, area, size_v, size_h, price, addtime, admin_id, provinceid, districtid, wardid, way_id, address, structure, price_time, bodytext, homeimgfile, homeimgthumb, showprice, group_config')
