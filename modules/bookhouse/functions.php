...
{
    global $db, $module_name, $module_info, $array_way, $array_groups, $global_config, $array_cat, $module_data, $lang_global;
    ...
    if (!empty($item['group_config'])) {
        $group_config = unserialize($item['group_config']);
        foreach ($group_config as $groupid => $exptime) {
            if ($exptime == 0 or $exptime > NV_CURRENTTIME) {
                $item['color'] = $array_groups[$groupid]['color'];
                break;
            }
        }
    }
