...
if ($order_id > 0 and $checkss == md5($order_id . $global_config['sitekey'] . session_id())) {
    ...
    // VujiTech coupons
    $result = $db->query('SELECT cid, amount FROM ' . $db_config['prefix'] . '_' . $module_data . '_coupons_history WHERE order_id=' . $data['order_id']);
    $data['coupons'] = $result->fetch();

    if ($data['coupons']) {
        $result = $db->query('SELECT * FROM ' . $db_config['prefix'] . '_' . $module_data . '_coupons WHERE id=' . $data['coupons']['cid']);
        $data['coupons_info'] = $result->fetch();
    }
    //
