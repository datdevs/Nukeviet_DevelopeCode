...
function uers_order($data_content, $data_order, $total_coupons, $order_info)
{
    ...
    if (is_numeric($total_coupons)) {
        ...
        $xtpl->assign('price_coupons', nv_number_format(0, nv_get_decimals($pro_config['money_unit'])));
        $xtpl->assign('price_total', nv_number_format($price_total, nv_get_decimals($pro_config['money_unit'])));
    }
    //
...
function payment($data_content, $data_pro, $data_shipping, $payment_supported, $intro_pay, $point)
{
    ...
    // VujiTech coupons
    if (!empty($data_content['coupons_info']) and $data_content['coupons_info']['type'] === 'g') {
        $xtpl->assign('GIFT', $data_content['coupons_info']['discount']);
        $xtpl->parse('main.gift');
    }
...
function email_new_order($content, $data_content, $data_pro, $data_table = false)
{
    ...
    if ($data_table) {
        ...
        if (!empty($data_content['coupon']) and $data_content['coupon']['type'] === 'g') {
            $xtpl->assign('GIFT', $data_content['coupon']['discount']);
            $xtpl->parse('data_product.gift');
        }
...
function coupons_info($data_content, $coupons_check, $error)
{
    ...
    if (!empty($data_content)) {
        ...
        // VujiTech coupons
        if ($data_content['type'] == 'p') {
            $data_content['discount_text'] = '%';
        } elseif ($data_content['type'] == 'f') {
            $data_content['discount_text'] = ' ' . $pro_config['money_unit'];
        } else {
            $data_content['discount_text'] = '';
        }
        $xtpl->assign('DATA', $data_content);
