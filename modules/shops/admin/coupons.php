...
// VujiTech coupons
// Cần kích hoạt dòng này chạy ở trang Mã giảm giá nếu cài mới module shops thì không cần
// $db->query("ALTER TABLE " . $db_config['prefix'] . "_" . $module_data . "_coupons CHANGE discount discount TEXT NOT NULL");
...
$xtpl->assign('ROW', $row);
$xtpl->assign('Q', $q);
$xtpl->assign('LINK_IMPORT', NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=coupons_import');
...
if ($show_view) {
    ...
    while ($view = $sth->fetch()) {
        ...
        if ($view['type'] == 'p') {
            ...
            $view['discount_text'] = ' ' . $pro_config['money_unit'];
        } else {
            $view['discount_text'] = '';
        }
